...
func (r *Router) Handle(c *Context) int {
	...
	// check status
	if DEBUG {
		if c.Res.Status != 0 && c.Res.Status != status {
			log.Println(LOG_TAG, "Route: status conflict!")
		}
	}
	c.Res.Status = status // always use return status
